...
Son güncelleme: 2026-08-31 · **F4 KABUL KAPISI GEÇİLDİ** (senaryo 2. koşum: 16.3 sn, yeniden başlatmasız)
...
| **O55** | **`errors.ts` ↔ sözlük simetrisi kontrol edilmiyor** | ✅ karar | ❌ | ❌ | B7`nin sınıfı: bir hata kodu dört sözlükte de tanımlı olabilir **ve** `KNOWN_ERROR_CODES``ta olmadığı için ölü kalabilir; ya da tersi — kodda tanımlı ama metni yok. Hiçbir kontrol bu asimetriye bakmıyor. `i18n:check` bakmaz (kod→sözlük yönünde çalışır ve bu kodlar dinamik çözülür), `i18n:dead-keys` de yakalayamaz. <br><br>**Teknik lider çalışan bir prototip yazdı** (scratchpad`te, 19/19 simetri doğruluyor): `errors.ts``ten `KNOWN_ERROR_CODES` bloğunu ayrıştır, `tr/desktop.json` `errors` altındaki BÜYÜK_HARF anahtarlarla iki yönlü karşılaştır. `check-data-wiring` biçiminde bir script`e dönüştürülüp `package.json``a bağlanmalı — negatif kontrolüyle birlikte. |
...
| **O57** | **B8 — Çakışma Kutusu açıkken yeni çakışma listeye düşmüyor** | ❌ | ❌ | ❌ | Rozet güncelleniyor ama liste "No conflicts" demeye devam ediyor; kapat-aç düzeltiyor. Düşük etki, net hata. |
| **O58** | **G1 — `commands/sync.rs` yorumu bayat** | — | — | — | "background loop is not started by this shell yet" diyor; O46 bunu değiştirdi. Tek satırlık yorum düzeltmesi — ama bu defterin O8/O9 dersi tam olarak buydu: bayat yorum bir sonraki şeridin görev tanımı oluyor. |
