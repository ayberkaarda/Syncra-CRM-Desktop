...
Son güncelleme: 2026-08-31 · F4 senaryosu koştu — KABUL GEÇMEDİ (O45 B1, O46 B2)
...
| **O41** | **Kanban kartlarında rozet yok** | ✅ | ❌ | ❌ | `deal.move` çevrimdışı en olası yazma, yani rozet oraya en çok ait olduğu yer; ama kart `features/deals/components/` içinde çiziliyor ve W2-B'nin yazma izni `features/*/pages/*` ile sınırlıydı. `crm.ts`'teki `boardCard()` de dönüş tipini `WithSyncState<DealCard>` yapıp alanı taşımalı. Küçük, tek şeritlik iş. |
...
| **O43** | **§7.2/8 — birleşik arama teslim edilmedi** | ❌ | ❌ | ❌ | Şartname §7.2: *"Command palette lokal FTS + online sunucu **birleşik** (kaynak etiketi)"*. Bugün `manifest.ts:202` → `'search.query': { kind: 'query', via: 'search + rows_by_client_ids' }`, yani masaüstü araması **yalnız yerel FTS**. Sunucu sonuçlarıyla birleştirme ve sonuçların kaynağını gösteren etiket **yok**. <br><br>Etkisi: çevrimiçiyken kullanıcı, aynaya henüz inmemiş kayıtları command palette'te bulamaz ve bunu anlamasının bir yolu yoktur — sessiz eksik sonuç. **F4 "§7.2 tamamı" kabulünü karşılamıyor.** |
| **O44** | **§7.2/6 — quick-capture penceresi yok** | ⚠️ şartname içi çelişki | ❌ | ❌ | §7.2 quick-capture penceresini F4 kapsamındaki UI listesine koyuyor, ama §10 aynı özelliği **F5 madde 3** (hotkey quick-capture) olarak listeliyor. Kod hiçbir yerde yok (grep 0). İki bölüm aynı işi iki faza yazmış; F5'te teslim edilecekse §7.2'ye dipnot düşülmeli, F4 kapsamındaysa F4 kapanamaz. **Kullanıcı kararı.** |

| **O45** | 🔴 **B1 — çevrimdışı `op=action` mutasyonlarının HİÇBİRİ uygulanmıyor** | ❌ | ❌ | ❌ | İstemci sözleşmeye uygun **çıplak** action gönderiyor (`"entity":"deal","action":"move"`); sunucu `MutationApplier.php:437` `$action = (string) $mutation->action` ile çıplak değeri alıp **noktalı** `ALLOWED_ACTIONS` (`'deal.move'`, `'task.complete'`, …) listesiyle karşılaştırıyor → `INVALID_MUTATION`. <br><br>**Sözleşme istemciyi haklı çıkarıyor:** `docs/DESKTOP-SYNC-PROTOCOL.md:352` wire örneği `"entity":"notification","action":"read_all"` — çıplak. <br><br>**12 fiilin tamamı ölü:** `deal.move` (F4'ün manşet özelliği), `task.complete`, `ticket.status`, `notification.read_all`, … Üç ardışık push ile kök neden kanıtlandı: çıplak → red; noktalı → beyaz listeyi geçti. <br><br>**Neden hiçbir test yakalamadı:** `SyncPushTest.php` fixture'ları **noktalı** gönderiyor (satır 304, 340, 381, 517…), yani sunucunun kendi konvansiyonunu test ediyor; crate testleri kendi tarafında çıplak gönderip yeşil. **İki yeşil takım, arada kırık wire — RISK-1'in dördüncü ve en pahalı tezahürü.** |
| **O46** | 🔴 **B2 — ağ dönünce uygulama kendiliğinden çevrimiçi olamıyor (kilitlenme)** | ⚠️ şartname sorunu | ❌ | ❌ | Üç şey üst üste biniyor: (1) `sync_now()` `!online` iken erken `Offline` dönüyor, yani bağlantının döndüğünü **keşfedemiyor**; (2) `online=true` yalnız `login`/`restore_session`/`set_online(true)` ile oluyor; (3) sonuncuyu çağıran **arka plan döngüsü hiç başlatılmıyor** — `start_background_sync` `sync/mod.rs:922`'de tanımlı ama `src-tauri`'den çağrılmıyor, `lib.rs:8` başlatmayı açıkça **F5-1'e** bırakıyor. Üstüne `ConnectivityBar.tsx:78` senkron düğmesini çevrimdışıyken **devre dışı** bırakıyor. <br><br>Tek çıkış: uygulamayı yeniden başlatmak. <br><br>**Bu bir şartname sıralama sorunu:** F4'ün kabulü ("ağ aç → ≤60 sn'de sunucuda") F5'e atanmış bir parçaya bağımlı. Eylem verildiğinde senkron **4.8 sn** sürüyor — yol hızlı ve doğru, ona ulaşmanın yolu yok. **Kullanıcı kararı:** döngü F4'e mi alınsın, yoksa F4 kabulü mü revize edilsin? |
| **O47** | **B3 — `task.complete` payload'ı eksik** | ❌ | ❌ | ❌ | `work.ts::complete()` → `runAction('task', id, 'complete')` `payload:{}` gönderiyor; sunucunun `CompleteTaskRequest`'i `completed` alanını **zorunlu** istiyor. O45 düzelse bile bu ayrıca patlar (kanıtlandı: noktalı ad + boş payload → "tamamlanma durumu alanı zorunludur"). `deal.move`'da bu sorun **yok**. |
| **O48** | **B4 — `INVALID_MUTATION` için i18n anahtarı yok** | ❌ | ❌ | ❌ | Çakışma Kutusu reddedilen mutasyonları *"An unknown error occurred."* diye gösteriyor ve satırlarda kayıt adı yerine `client_id` UUID'si yazıyor. Kullanıcı neyin neden reddedildiğini anlayamıyor. |
