...
| §6.5 | O12 | Updater bloğu `tauri.conf.json`'a girdi (gerçek minisign pubkey `AD95461A5419A14A`, GitHub Releases endpoint'i, `installMode: "passive"`); `lib.rs`'teki `#[cfg(not(debug_assertions))]` sarmalayıcısı **silindi** — CI `--debug` derlediği için o blok hiç derlenmiyordu, yani panik sınıfı yapısal olarak görünmezdi | 2026-09-01 |
