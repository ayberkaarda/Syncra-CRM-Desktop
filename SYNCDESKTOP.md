...
<a id="k-a29-attachment-meta"></a>
**KARAR A29 (defter O90) — mesaj eki METADATA'sı pull satırındadır; `attachments` sync kapsamına GİRMEZ.** Sunucu `messages` pull satırına dört alan ekler (`SyncPullService::attachMessageAttachments()`, **A26 deseni**; sayfa başına tek sorgu, N+1 yok): `attachment_name`, `attachment_mime`, `attachment_size`, `attachment_is_image`. Alanlar `ChatAttachmentResource::payload()`'dan düzleştirilir — `is_image` tanımı istemcide yeniden türetilmez (**K7**). `deleted_at` dolu satıra **eklenmez** (`MessageResource` maskesiyle parite). Satır kapsamı `SyncScope`'un üyelik filtresinden geçtiği için `AttachmentPolicy` ile birebir aynıdır — **yeni sızıntı yüzeyi yoktur**.

Dosya **BAYTLARI senkronlanmaz**; `attachments` protokol §1.3'te kalır, aynada `attachments` tablosu açılmaz. Ayna tarafı: `migrations/0004_message_attachment_fields.sql` (dört kolon). İstemci: `mapMessage()` bu alanlardan `Attachment` DTO'su kurar.

**Masaüstünde inline görsel önizleme AÇIK maddedir ve bu bilinçlidir (defter O90b).** Ölçüldü: `<img src>` isteği **bearer başlığı taşıyamaz** ve masaüstü cookie göndermez (`desktop/src/platform/http.ts` → `transport: 'bearer'`, `withCredentials` kapalı; A12 origin'in stateful olmasını bilerek yasaklıyor), rota ise `auth:sanctum` arkasındadır → **401**. Ayrıca `ChatAttachmentResource` url'i host'suzdur ve webview origin'i `http://tauri.localhost`'tur. Bu yüzden mapper şimdilik `is_image: false` düşürür ve her ek **ad + boyut taşıyan dosya kartı** olarak çizilir — kart ağ istemez, **çevrimdışı da görünür**. `attachment_is_image` kolonu yine de aynalanır ki önizleme kanalı ileride açıldığında yeniden pull gerekmesin.

Mevcut ekli mesajlar tek seferlik backend migration'ı ile yeniden versiyonlanır (**K5** — satır başına benzersiz; `SyncVersionBackfill` deseni kopyalanır, `run()` çağrılmaz çünkü içindeki `WHERE sync_version = 0` filtresi bu satırları tutmaz).
...
`auth::{login, session, restore, logout, list_devices, revoke_device}` · `data::{query, mutate, search}` · `sync::{sync_now, status, conflicts, resolve_conflict, download_archive, bootstrap, handle_realtime}` · `storage::{storage_stats, storage_settings, update_settings, clear_local, data_location, move_data_dir}` · `files::{cache_quote_pdf, open_cached, attach_from_paths, screenshot_to_ticket}` · `os::{set_badge, register_hotkey, set_autostart, get_autostart, set_tray_language, notify, record_opened}`.

> **`os::record_opened` (2026-09-01, O85).** JumpList'in "son 5 kayıt" beslemesi. İmza `record_opened(entity, id, title, categoryLabel)`; `entity` + `id` doğrulaması **ikinci bir tablo kurmuyor** — `deep_link::parse_deep_link`'e veriliyor, yani sekiz isimlik allowlist ve `^[0-9]{1,12}$` id deseni O89'un fuzz'la çivilenmiş parser'ından geliyor. `title` ve `categoryLabel` webview'den gelir: kabuğun başlığı yoktur ve i18n webview'da yaşar (`os::notify` ile aynı gerekçe); Rust'ta aynadan başlık okumak entity'ye göre değişen kolon adlarının ikinci bir el-tablosunu doğururdu. Non-Windows'ta komut **vardır ve aynı doğrulamayı yapar** (`{code,message}` sözleşmesi platforma göre değişmez), ama hiçbir şey saklamaz — menüsü olmayan bir makinede beş kayıt başlığını düz metne yazmak, özelliğin gizlilik maliyetini ödeyip özelliği vermemek olurdu.
...
`core:default, core:window:allow-*state*, notification:default, global-shortcut:allow-register/unregister, deep-link:default, autostart:default, updater:default, ~~clipboard-manager:allow-read-text~~ **VERİLMEZ** (2026-09-01 düzeltmesi: burada yazan "yalnız clipboard opt-in aktifken runtime izin" düzeneği **hiç yazılmadı**; kod şartnameden bilinçli olarak DAR ve §9/6'nın kanıtı tam buna dayanıyor — webview'e pano okuma izni verilmediği `capabilities/default.json` parse edilerek testle çivilendi. Pano yalnız Rust tarafından, kullanıcının opt-in'iyle okunur.), dialog:allow-open, fs: scope = [$APPDATA/syncra/**, $TEMP/syncra/**] + dialog ile seçilen`.
...
- `<api-host>`/`<reverb-host>` yer tutucudur — **KARAR D-3 rev.2 (2026-09-01): build-time değil, launch-time.**
  > **Eski öncül çürütüldü.** D-3 ve §10 F8/3, *"Tauri'de CSP runtime'da değiştirilemez, dolayısıyla runtime yapılandırılabilir API URL'i `connect-src` jokerini zorunlu kılar"* varsayımına dayanıyordu. Bu **yanlış ikilemdi** ve `tauri 2.11.5` kaynağı teknik lider tarafından okunarak çürütüldü:
  > `manager/mod.rs:371` politikayı **her sayfa servisinde** `self.config.app.security.csp`'den okuyor · `protocol/tauri.rs:183` onu bir HTTP **response header'ı** olarak basıyor · `lib.rs:411` `pub fn config_mut(&mut self) -> &mut Config` — *"A mutable reference to the config the application was prepared with"* — yani `Builder::build`'den **önce** politika belirlenebilir.
  > Doğru ifade: **`tauri.conf.json`'daki CSP paket sonrası düzenlenemez; ama politika süreç başına bir kez, başlatmada sentezlenebilir.** Build sonrası mutasyon yolu yoktur (`AppManager::config()` yalnız `&Config` verir, `set_csp` `pub(crate)`), dolayısıyla adres değişikliği **relaunch** ister (`process:allow-restart` capability'de mevcut).
  >
  > **Sonuç:** origin'in *ne zaman* bağlandığı değişir (derleme → başlatma), *ne kadar dar* olduğu değişmez. `connect-src` her zaman **tam origin çifti** kalır; joker (`https:`, `*`) **hiçbir yönergede YASAK**.
...
(`desktop-release.yml` `includeUpdaterJson: true` + `updaterJsonPreferNsis: true` taşıyor.) Kanal `stable`. Güncelleme sync döngüsü boşta iken uygulanır.

> **DÜZELTME (2026-09-01, TM-F16 / defter O108).** Bu cümle *"zaten üretip imzalıyor"* diyordu ve **yanlıştı**: `bundle.createUpdaterArtifacts` konfigürasyonda **yoktu** ve varsayılanı `false`'tur, yani `tauri build` ne `.sig` ne updater artifact'i üretiyordu — `includeUpdaterJson`'ın imzalayacağı hiçbir şey oluşmuyordu. Tahmin değil, ölçüm: 2026-09-01 bundle'ı tam olarak bir `.msi` ve bir `-setup.exe` içeriyordu, başka hiçbir şey. Yani O12 ile girilen updater bloğu, **hiçbir zaman var olmayacak** bir manifest'i işaret ediyordu.
>
> Alan `true` yapıldı. Bunun ölçülen bedeli: `createUpdaterArtifacts` açıkken bundler **özel anahtar olmadan bitmeyi reddediyor** (*"A public key has been found, but no private key"*, exit 1) — ve bu, `desktop-ci.yml`'nin her push'ta koşan imzasız `tauri build --debug` işini de kırıyordu. Çözüm: o iş `--config scripts/ci-unsigned-bundle.json` ile **yalnız kendisi için** alanı `false`'a çeviriyor; böylece bundler test edilmeye devam ediyor, imzalama ise secret'ı tutan tek yere, `desktop-release.yml`'ye kalıyor. Üçü de ölçülerek doğrulandı (imzasız release build exit 1 · imzasız debug build exit 1 · overlay ile debug build **exit 0**, iki bundle üretildi).
...
| 5 | **Deep link reddi (fuzz 83 örnek)** — korpus, plugin'in verdiği **ayrıştırılmış `url::Url`** katmanından beslenir (`Url::parse` → `as_str()`, plugin'in `handle_cli_arguments`'ıyla **birebir aynı** `url 2.5.8`; `Cargo.lock` doğrulandı). Ham dizgi üzerinden test etmek yanıltıcıydı: `deal/../29` ham hâlde reddedilir, gerçek yolda normalizasyon `..`'yi siler ve hedef kabul edilir (defter O89). <br><br>**İddia:** normalizasyon sonrası ne kalırsa kalsın, emit edilen hedef regex + sekiz-isim allowlist'inden geçmiştir; her örnek **iki** meşru sonuçtan birine düşer (red · sözleşme içi `{entity, id}`), **üçüncüsü yoktur.** Ölçüm: **4 ayrıştırılamaz · 57 red · 22 normalizasyondan sağ çıkıp sözleşme içinde.** İddia, test içinde **bağımsız** bir `CONTRACT_ENTITIES` + elle yazılmış şekil kontrolüyle kuruldu — `ENTITIES`'e sormak totoloji olurdu, allowlist genişleyince assert de sessizce genişlerdi. <br><br>**Normalizasyon `id`'yi değiştirebilir, `entity`'yi asla:** `deal/1/../2` → deal **2** (linkte görünen id, açılan id değil); `deal/9999999999999/../42` → deal 42, regex'in reddedeceği 13 haneli segment sonraki segmentle siliniyor. Ama entity url'in **host**'udur ve hiçbir path normalizasyonu / nokta segmenti / percent-encoding host'a ulaşmaz — `deal/../lead/1` host'u `deal` bırakır (sonra red), `setting/../deal/42` allowlist'e takılır, `DEAL/42` küçük harfe çevrilmediği için reddedilir. **KARAR: düzeltilmez.** Kabuğun kendi URL normalizasyonunu yazıp `url` crate'ini ikinci kez tahmin etmesi daha kötü bir sınırdır (parser-differential); allowlist entity üzerinden sağlam ve id kayması yalnız aynadaki, yani `SyncScope`'un pull anında filtrelediği kayıtlara ulaşabilir. Davranış testle çivilendi. <br><br>**İkinci savunma hattı kapalı DEĞİLDİ, kapatıldı:** `deeplink-routes.ts` `ROUTES[entity]` düz property lookup olduğu için **prototip zincirini** yürüyordu (`constructor` boxed `String` döndürüyor, `__proto__`/`valueOf`/`hasOwnProperty` Tauri callback'i içinde `TypeError` fırlatıyordu) ve **id hiç doğrulanmıyordu** (`../admin` → `/deals/../admin`). `Object.hasOwn` + `^[0-9]{1,12}$` ile kapatıldı; kabul edilen küme daralmadı. <br><br>**Negatif kontroller koşuldu:** `ENTITIES`'e dokuzuncu isim eklenince fuzz **5 testle kırmızı**; TS guard'ları kaldırılınca 2 test kırmızı. | ✅ **KAPALI** |
| 6 | Clipboard içeriği log/diske yazılmıyor | ✅ **KAPALI (2026-09-01)** — F5-6 kodu geldiği için artık değerlendirilebilirdi ve **ölçüldü**. Pano metninin (`clipboard.rs:204`) izlediği tüm yol çıkarıldı: yalnız `fingerprint()` (u64), `detect()` (alansız enum), `drop`. Modülde `fs`/`log`/`println!`/`emit` yok; tek `tracing::warn!` bildirim plugin'inin hatasını yazıyor. **Bağımlılık katmanı da tarandı**: `arboard 3.6.1`'in Windows'taki tek `log::` satırı `GlobalUnlock` hatasını yazıyor, içeriği değil; `clipboard-win 5.4.1` ve `tauri-plugin-clipboard-manager 2.3.2`'de `log::` hiç yok; uygulamada panic hook kurulmuyor. <br><br>**Tarayıcının kendisinde iki gerçek delik bulundu ve kapatıldı:** (a) yalnız `tracing::` içeren satırlara bakıyordu — `log::info!`, `println!`, `format!`, `.emit()`, `.body()` taramanın dışındaydı; (b) **satır bazlıydı**, yani `tracing::warn!(` açılıp `%text` alt satıra yazılsa görmezdi. Bugün 5 test var, biri **ters yönlü kilit** (sink listesinden değil panodan başlar: `text`'i anan her üretim satırı izinli çağrı listesinde olmak zorunda), biri de `capabilities/default.json`'ı **parse edip** webview'e `clipboard*` grant'ı verilmediğini assert ediyor — daha önce bu yalnız `description` düzyazısında yazan bir iddiaydı. 4 negatif kontrol koşuldu. |
| 7 | CSP ve capabilities dar; `shell` yalnız `open` | ✅ KAPALI (bugünkü yüzey için) |
| 8 | Updater imza doğrulaması; imzasız manifest reddi | ✅ **KAPALI — NİTELEMELİ (2026-09-01)** — O12 bloğu girdiği için değerlendirilebilir hâle geldi. `tauri-plugin-updater 2.10.1` kaynağında red yolları `dosya:satır` ile çıkarıldı: imzasız manifest `platforms` biçiminde serde'de düşer (`updater.rs:76`, indirme **başlamaz**), düz biçimde *"the `signature` field was not set"* (`:1422-1424`), yanlış anahtar/bozuk payload `PublicKey::verify` → `Error::Minisign` (`:1461`), anahtarsız konfigürasyon deserialize olmaz (`config.rs:101,122`). Yapısal olarak: `verify_signature`'ın **tek** çağrı yeri `download`'un buffer'ı döndürmesinden hemen öncedir (`:712`) ve `install()` yalnız `download()`'un döndürdüğünü alır → doğrulanmamış bayt kurulamaz; hiçbir `dangerous*` bayrağı imzayı gevşetmez. `src-tauri/src/updater.rs` (üretim kodu yok, yalnız 6 test) bunu plugin'in **kendi `Config` deserializer'ıyla** ölçüyor: pubkey dolu, tek endpoint, şema `https`, üç `dangerous*` kapalı, pubkey iki base64 katmanı çözülüp gerçek minisign anahtarı olduğu (42 bayt, `Ed` + key id + Ed25519, key id comment'iyle birebir `4CE97F70B4BEFE48`) doğrulanıyor. `pubkey` çıkarılınca `Config` deserialize olmuyor — fail-closed **ölçüldü**. <br><br>**KALAN YARISI DA ÖLÇÜLDÜ — 2026-09-02, ilk gerçek release (run 33571076974, üç OS yeşil, taslak sürüm `desktop-v0.1.0`).** Üretilenler: **14 varlık**, bunların **6'sı `.sig`** (msi · nsis · deb · rpm · AppImage · app.tar.gz) ve **`latest.json` (5328 bayt, 9 platform girdisi, hepsinde imza)**. Teknik lider manifest'i indirip doğruladı: `windows-x86_64` girdisinin imzası `Syncra_0.1.0_x64-setup.exe.sig` dosyasıyla **birebir aynı** (yer tutucu değil), ve imzanın ikili gövdesinden çıkarılan anahtar kimliği **`4CE97F70B4BEFE48`** — yani `tauri.conf.json`'a gömülü ve `updater.rs`'in `EXPECTED_KEY_ID`'siyle çivilenen anahtarın **ta kendisi**. `updaterJsonPreferNsis` de çalışmış: `windows-x86_64` ile `windows-x86_64-nsis` aynı varlığı gösteriyor. <br><br>**Hâlâ ölçülmeyen, dürüstçe ayrılıyor:** *bir güncellemenin gerçekten kurulduğu*. Bunun için ikinci bir sürüm ve onu çeken bir istemci gerekir; tek sürümle kanıtlanamaz. Bugün kanıtlanan şey, uygulamanın çekeceği manifest'in **var olduğu, imzalı olduğu ve doğru anahtarla imzalandığı**. <br><br>*(Aşağıdaki eski kayıt, ölçümden önceki durumu anlatır:)* **Kalan doğrulama, açıkça ayrılmıştı:** *"doğru imzalı güncelleme kurulur"* ve *"yolda bozulan payload reddedilir"* **DOĞRULANMADI** — bu build'in kabul edeceği bir `latest.json` üretmek özel anahtarı gerektirir, o anahtar bu ağaçta yok ve olmayacak. Sahte imza üretip reddini göstermek `minisign-verify`'ın test kapsamıdır, bizim konfigürasyonumuzun değil. İlk gerçek imzalı sürümde ölçülecek; **ön koşulu O108'di ve kapandı** (`bundle.createUpdaterArtifacts`). |
...
> **ÖLÇÜLDÜ (2026-09-01, kurulu NSIS paketi üzerinde — defter O103):**
> • **MSI:** `Syncra_0.1.0_x64_en-US.msi` **12,92 MB** (13.549.568 bayt) → **GEÇTİ**. NSIS setup 7,56 MB.
> • **Boşta RAM:** 7 süreçlik ağacın toplam Private Bytes'ı t=0/90/180 sn'de **214,2 · 211,1 · 212,3 MB** (Working Set ~435 MB) → **GEÇTİ**, üç noktada düz. Kırılım kriterin gerekçesini doğruluyor: ana süreç yalnız **8 MB**, kalan ~204 MB altı WebView2 sürecinde.
> • **Soğuk başlangıç deep link:** uygulama kapalıyken `syncra://deal/29` uygulamayı açtı **ve kaydın detayına gitti** (ekran görüntüsüyle) → **GEÇTİ**.
> • **Şema kaydı:** `HKCU\Software\Classes\syncra\shell\open\command` NSIS kurulumundan geliyor → **GEÇTİ**, `register_all()` gereksiz.
> • **JumpList "son 5 kayıt" (O85) — YAZILDI ve ÖLÇÜLDÜ.** Ölçüm sırasında kodun **hiç yazılmadığı** saptandı (madde "paketlenince doğrulanır" sanılıyordu) ve aynı turda teslim edildi. Kurulu pakette: `recent.json` **tam 5 giriş**, en yenisi başta, altıncı düşmüş, kategori yerelleştirilmiş; başlıkların bir kısmı aynadan gerçek metin, bir kısmı fallback — iki yol da tetiklendi. **Windows listeyi kabul etti**: `CustomDestinations` 21 → 22, yeni 9,1 KB'lık depo dosyası. **Gizlilik kancası uçtan uca kanıtlandı** (defter O107): `clear()` sonrası o dosya **kayboldu**. Açılış rebuild'i de ölçüldü. <br>&nbsp;&nbsp;**Ölçülmeyen, dürüstçe ayrılıyor:** menünün görsel hâli, girişe tıklayınca kaydın açılması, "Remove from this list"e saygı — üçü de insan etkileşimi ister.
> • **CI KOŞTU ve YEŞİL — 2026-09-01, run 33550538821, 10/10 iş.** Bu dalın tarihinde `tauri build --debug` ve `release smoke` **ilk kez** çalıştı. Smoke'un kendi çıktısı: `Started …/release/syncra-desktop.exe (pid 8328); waiting up to 10s...` → `Still running after 10s - release binary starts cleanly.` **F7'nin kabul kriteri karşılandı.**
> • Dört tur sürdü ve üçü konfigürasyon hatasıydı; anatomisi defter O115'te — özellikle son ikisi teknik liderin hatasıydı ve **YAML ayrıştırma + `bash -n` ikisini de yeşil geçti**, çünkü ikisi sözdizimine bakar, bağlama değil.
...
> **ZORUNLU — taşımadan önce `SyncEngine::shutdown()` çağrılır (defter O104, ölçüldü 2026-09-01).** `SyncScheduler::stop()` **tek başına yetmez**: `stop()` yalnızca `abort()`'tur ve senkron değildir; arka plan task'ının tuttuğu `SyncEngine` klonu (dolayısıyla SQLite bağlantısı) bir süre daha yaşar. Windows'ta açık tutamaklı bir dizin **silinemez** — `remove_dir_all` OS hata 32 döner — yani "eskiyi sil" adımı rastgele düşer ve kabul kriteri ("eski dizin **boş**") sağlanamaz. `shutdown()` deterministiktir: `PRAGMA wal_checkpoint(TRUNCATE)` + `conn.close()`. Kanıt: `tests/engine_loop.rs::shutdown_frees_the_data_directory_for_the_f8_migration` (kapatma sonrası tüm dizin silinebiliyor) + negatif kontrol (açık motorla aynı çağrı OS hata 32 veriyor). `src-tauri/src/lib.rs`'in `RunEvent::Exit` yolu bu sırayı zaten uyguluyor.
>
> **Kopyalanacak dosyalar (ölçüldü):** `syncra.db` **zorunlu** · `syncra.db-wal` **varsa zorunlu** — temiz kapanıştan sonra var olmaz ama önceki koşum çökerek bittiyse commit edilmiş veri taşır, atlanırsa **veri kaybı** olur · `syncra.db-shm` gereksiz (SQLite yeniden üretir) · `cache/` **zorunlu** (`cached_files` defteri o yola adres veriyor; kopyalanmazsa defter var olmayan dosyaları gösterir). Pratik biçim: `syncra.db*` glob + `cache/`.
>
> **Hata yolu:** "eskiyi sil" adımı başarısız olursa **sessizce yutulmaz** (`let _ =` yasak) — kullanıcıya eski verinin hâlâ nerede durduğu söylenir. Aksi hâlde şifreli ayna iki yerde kalır; bu bir tehdit modeli maddesidir, kozmetik değil.
...
3 **Runtime API URL — KARAR TURU YAPILDI 2026-09-01, KARAR: launch-time CSP sentezi (D-3 rev.2).**

**Reddedilen üç seçenek ve gerekçeleri:**
- **(a) Kalıcı per-backend build.** Sürdürülemez, ve **updater'la yapısal olarak çelişiyor** (bkz. defter O111): `tauri.conf.json` tek sabit endpoint + tek pubkey taşır, yani self-hoster'ın kendi derlediği binary de onları taşır ve ilk resmî sürümde `installMode: "passive"` onun özel-host build'ini **bizim build'imizle sessizce ezer**. Ayrıca `desktop-release.yml` tek `secrets.DESKTOP_RELEASE_ENV` varsayımıyla kurulu.
- **(b) `connect-src` genellemesi (`https:` / `*`).** Mimarinin son savunma hattını satar. Bugün XSS'e karşı beş katman var (HTMLPurifier fail-closed · `dangerouslySetInnerHTML` **fiili kullanım 0** · tek HTML render noktası `<iframe sandbox="">` · script'te `unsafe-inline` yok · token yalnız bellekte) — ama hepsinin arkasındaki değişmez *"XSS olsa bile sızdıracak kanal dar"*. Tehdit modeli I5, T4 ve §3/7 birlikte açılırdı. **REDDEDİLDİ.**
- **(c) HTTP'yi Rust'a taşımak.** Hedefe **zaten ulaşmıyor**: Reverb websocket'i webview'da yaşıyor (`desktop.ts` Echo `wsHost`) ve `img-src` ek/avatar için host'a bağlı — 30 `kind:'http'` fiil + upload + blob taşınsa bile CSP host'tan kurtulmaz. `tauri-plugin-http` CSP'yi baypas eder (istekleri Rust'ta `reqwest` ile atar) ama scope'u `https://**` yapmak (b)'nin sızdırma kanalını IPC üzerinden geri açar — maliyeti katmandan katmana taşımaktan başka şey yapmaz. **REDDEDİLDİ.**

**KARAR (d3):** API + Reverb adresi kullanıcı yapılandırması olur; `lib.rs::run()` içinde **`Builder`'dan önce** okunur, doğrulanır (https zorunlu, loopback istisnası — `check:release-host` sınıflandırmasının Rust aynası), CSP ondan **jokersiz** sentezlenir ve `context.config_mut().app.security.csp` ile bağlanır. `state.rs` api_base aynı kaynaktan beslenir; `option_env!` yalnız dev fallback kalır. Adres değişikliği relaunch ister.

**Bu madde bir KARAR turudur — kod bu fazın kapsamında değildir.** Uygulama iş kalemleri ve çözülmemiş alt kararı (`VITE_REVERB_APP_KEY` de build-time gömülü; self-host'ta sunucudan keşfedilmeli — defter O112) defterde izleniyor. **Uygulanmadan önce revize edilecek belgeler:** §6.3 (yapıldı, D-3 rev.2) · `docs/DESKTOP-ARCHITECTURE.md` §11 D-3 · `docs/DESKTOP-THREAT-MODEL.md` TM-F6 gerekçesi + yeni **TM-F17** (güven çapası build-time `.env`'den runtime yapılandırma deposuna taşınıyor).
...
| §6.5 | O12 | Updater bloğu `tauri.conf.json`'a girdi (gerçek minisign pubkey `4CE97F70B4BEFE48`, GitHub Releases endpoint'i, `installMode: "passive"`); `lib.rs`'teki `#[cfg(not(debug_assertions))]` sarmalayıcısı **silindi** — CI `--debug` derlediği için o blok hiç derlenmiyordu, yani panik sınıfı yapısal olarak görünmezdi | 2026-09-01 |
| §10 F7 | O103 | Kabul kriterlerinin ölçüm sonuçları eklendi: MSI 12,92 MB · boşta RAM 214/211/212 MB · soğuk başlangıç deep link · şema kaydı. JumpList'in **kodunun hiç yazılmadığı** kayda geçti (madde "paketlenince doğrulanır" sanılıyordu) | 2026-09-01 |
| §8 | O102 | Liste bir sözleşme olarak duruyordu ama **uygulanmamıştı**: `onlineOnly()` yardımcısı ve `OnlineOnlyError` tanımlı, 17 tooltip anahtarı 4 dilde hazır, çağrı sayısı **0**. Bağlama işi açıldı | 2026-09-01 |
| §9 madde 6 | F6 / pano | ⬜ DEĞERLENDİRİLEMEZ → ✅ KAPALI: pano metninin izlediği yol bağımlılıklar dahil ölçüldü; tarayıcının kendi iki deliği (yalnız `tracing::`, satır bazlı) bulunup kapatıldı, 5 test + 4 negatif kontrol | 2026-09-01 |
| §9 madde 8 | O12 / O108 | ⬜ DEĞERLENDİRİLEMEZ → ✅ KAPALI (nitelemeli): red yolları `dosya:satır` ile, anahtarsız konfigürasyonun reddi plugin'in kendi deserializer'ıyla ölçüldü. Uçtan uca imzalı akış **ölçülmedi** ve öyle yazıldı | 2026-09-01 |
| §6.5 | O108 / TM-F16 | *"`desktop-release.yml` zaten üretip imzalıyor"* iddiası **yanlıştı** — `bundle.createUpdaterArtifacts` yoktu, varsayılanı `false`; üretilen bundle'da tek `.sig` yok. Alan açıldı, CI'ın imzasız işi `--config` overlay'iyle korundu | 2026-09-01 |
| §6.3 | D-3 rev.2 / O110 | `<api-host>` **build-time → launch-time**. *"Tauri'de CSP runtime'da değiştirilemez"* öncülü `tauri 2.11.5` kaynağıyla çürütüldü (`manager/mod.rs:371`, `protocol/tauri.rs:183`, `lib.rs:411`). `connect-src` yine jokersiz tam origin | 2026-09-01 |
| §6.3 | O102 / §9-6 | `clipboard-manager:allow-read-text` **VERİLMEZ** olarak düzeltildi — "runtime izin" düzeneği hiç yazılmadı, kod şartnameden bilinçle dar ve §9/6'nın kanıtı buna dayanıyor | 2026-09-01 |
| §10 F8/1 | O104 | Taşıma prosedürüne zorunlu madde: `SyncScheduler::stop()` tutamağı bırakmaz, `SyncEngine::shutdown()` **şart**; kopyalanacak dosya listesi ölçülerek yazıldı | 2026-09-01 |
| §10 F8/3 | O110 | Karar turu **yapıldı**: launch-time CSP sentezi. (a) per-backend build updater'la yapısal olarak çelişiyor (O111), (b) son savunma hattını satıyor, (c) hedefe zaten ulaşmıyor — üçü de gerekçeli reddedildi | 2026-09-01 |
...
| `#k-a29-attachment-meta` | §4.4 | A29 — mesaj eki metadata'sı pull satırında |
